  ...
  client.on("messageCreate", async (message) => {
    try {
      ...
      if (message.content.length > msgLengthLimit) {
        message.reply(
          "Whoa now, I'm not going to read all that. Maybe summarize?"
        );
        return;
      }
      ...
      let conversationLog = [
        {
          role: "system",
          content:
            "People refer to you as Natalie Sakurai and you are an assistant of Madaleine on his server called Madaleine HQ.",
        },
      ];
      ...
      if (reply?.length > 2000) {
        ...
        const txtFile = new AttachmentBuilder(buffer, {
          name: `${message.author.tag}_response.txt`,
        });
